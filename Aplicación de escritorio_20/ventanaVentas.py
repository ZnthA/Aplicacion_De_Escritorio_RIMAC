...
class VentanaVentas(QtWidgets.QMainWindow):
    ...
    def __init__(self,parent = None):
        ...
        uic.loadUi("ventanaVentas.ui", self)
        ...
        if self.txtCodigoProducto.text() == "":
            ...
            if pos == -1:
                ...
                if stock_temporal == -1:  # ocurre cuando no existe el producto en el diccionario
                    self.stock_actual_temporal[self.txtCodigoProducto.text()] = objProducto.getStockActual()
                    stock_temporal = self.obtenerStockActualTemporal(codigoProducto)  
    ...
    def actualizarStockActualTemporal(self, codigoProducto, cantidad):
        print('stock_actual_temporal', self.stock_actual_temporal, cantidad)
        for llave in self.stock_actual_temporal:
            if llave == codigoProducto:
                self.stock_actual_temporal[llave]= int(self.stock_actual_temporal[llave]) - int(cantidad)
    ...
    def agregar(self):
        try:
            print('IF', self.txtCantidad.text(), self.txtStock.text())
            ...
            print('Exception', TypeError, NameError)
